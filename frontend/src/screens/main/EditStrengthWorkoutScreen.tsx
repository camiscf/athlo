...
import { StrengthActivity, ExerciseLog } from '../../types';
import DatePicker from '../../components/DatePicker';
import TimePicker from '../../components/TimePicker';

interface EditStrengthWorkoutScreenProps {
  route: {
    params: {
      activityId: string;
    };
...
export default function EditStrengthWorkoutScreen({ route, navigation }: EditStrengthWorkoutScreenProps) {
  ...
  async function loadActivity() {
    try {
      ...
      // Preencher duração
      if (data.duration) {
        const durationH = Math.floor(data.duration / 3600);
        const durationM = Math.floor((data.duration % 3600) / 60);
        setDurationHours(durationH > 0 ? String(durationH) : '');
        setDurationMinutes(durationM > 0 ? String(durationM) : '');
      }
  ...
  async function handleSave() {
    if (exercises.length === 0) {
      showAlert('Erro', 'Nenhum exercício para salvar.');
      return;
    }
    ...
    try {
      ...
      await api.updateStrengthActivity(activityId, data);
      showAlert('Sucesso', 'Treino atualizado com sucesso!');
      navigation.goBack();
    } catch (error: any) {
    ...
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.content}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{activity?.title || activity?.division_name || 'Treino'}</Text>
